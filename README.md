...
| `BMO_STEP_MIN_MS` / `BMO_STEP_MAX_MS` | Per-step simulated work range (default `700`/`2100` ≈ 0.7–2.1s, so transitions are observable). Each handler logs `"[fn] simulating work for N.Ns"`. Set both to `0` for fast runs. |
